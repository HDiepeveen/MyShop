...
public static class ClearProductVariantSkuEndpoint
{
    public static IEndpointRouteBuilder MapClearProductVariantSku(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapDelete(
                ...
                ExecuteAsync)
            .WithName("ClearProductVariantSku");
        ...
        ExecuteAsync(Guid productId, Guid variantId, [FromServices] UseCase useCase, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var result = await useCase.ExecuteAsync(
                new ClearProductVariantSkuCommand(ProductId.From(productId), ProductVariantId.From(variantId)),
                cancellationToken);

            return result.Failure switch
            {
                ClearProductVariantSkuFailure.ProductNotFound => TypedResults.NotFound(new ProblemDetails
                {
                    Title = "Product not found",
                    Detail = $"Product '{productId}' does not exist."
                }),
                ClearProductVariantSkuFailure.VariantNotFound => TypedResults.NotFound(new ProblemDetails
                {
                    Title = "Product variant not found",
                    Detail = $"Variant '{variantId}' does not exist on product '{productId}'."
                }),
                null => TypedResults.NoContent(),
                _ => throw new InvalidOperationException($"Clear SKU failure '{result.Failure}' is not supported.")
            };
        }
