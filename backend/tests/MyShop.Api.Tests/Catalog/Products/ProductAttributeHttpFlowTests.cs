...
public sealed class ProductAttributeHttpFlowTests
{
    [Theory]
    ...
    public async Task BoundEndpoints_RoundTripAllTypesAndKeepRevisionForEqualValues(AttributeDataType type, string jsonValue)
    {
        ...
        var original = Assert.Single(repository.Product.AttributeValues);
        var revision = repository.Token.Revision;
        Assert.NotEqual(initialRevision, revision);
        Assert.Equal(1, repository.SaveCalls);
        var get = await scenario.Send("GetProductAttributeValue");
        ...
        Assert.Same(original, Assert.Single(repository.Product.AttributeValues));
    ...
    public async Task BoundEndpoint_RejectsInvalidJsonWithoutChangingStoredValue(string body)
    {
        ...
        scenario.Repository.Product.SetAttributeValue(original);
        Assert.Equal(400, (await scenario.Send("SetProductAttributeValue", body)).Response.StatusCode);
        Assert.Equal(0, scenario.Repository.ReadCalls);
        Assert.Equal(0, scenario.Repository.SaveCalls);
        Assert.Same(original, Assert.Single(scenario.Repository.Product.AttributeValues));
    }
    ...
    {
        await using var scenario = new AttributeHttpScenario(AttributeDataType.Text, AttributeScope.Product);
        Assert.Equal(204, (await scenario.Send("SetProductAttributeValue", "{\"dataType\":0,\"value\":\"Red\"}")).Response.StatusCode);
        var revision = scenario.Repository.Token.Revision;
        Assert.Equal(204, (await scenario.Send("SetProductAttributeValue", "{\"dataType\":0,\"value\":\"Blue\"}")).Response.StatusCode);
        Assert.NotEqual(revision, scenario.Repository.Token.Revision);
        revision = scenario.Repository.Token.Revision;
        Assert.Equal(409, (await scenario.Send("SetProductAttributeValue", "{\"dataType\":1,\"value\":1}")).Response.StatusCode);
        Assert.Equal(revision, scenario.Repository.Token.Revision);
        Assert.Equal(2, scenario.Repository.SaveCalls);
        Assert.Equal("Blue", Assert.IsType<TextAttributeValue>(Assert.Single(scenario.Repository.Product.AttributeValues)).Value);
    }
}
