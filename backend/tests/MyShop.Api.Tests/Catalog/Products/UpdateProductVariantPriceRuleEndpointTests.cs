...
public sealed class UpdateProductVariantPriceRuleEndpointTests
{
    [Fact]
    public void MapUpdateProductVariantPriceRule_NullBuilder_Throws() =>
        Assert.Throws<ArgumentNullException>(() => UpdateProductVariantPriceRuleEndpoint.MapUpdateProductVariantPriceRule(null!));

    [Fact]
    public void MapUpdateProductVariantPriceRule_MapsNamedPutRouteAndReturnsSameBuilder()
    {
        var builder = WebApplication.CreateBuilder();
        var app = builder.Build();

        var returned = app.MapUpdateProductVariantPriceRule();
        ...
        Assert.Equal("UpdateProductVariantPriceRule", endpoint.Metadata.GetMetadata<IEndpointNameMetadata>()!.EndpointName);
        Assert.Equal(["PUT"], endpoint.Metadata.GetMetadata<IHttpMethodMetadata>()!.HttpMethods);
    }

    [Fact]
    public async Task ExecuteAsync_UpdatesRuleAndReturnsNoContent()
    {
        var scenario = new Scenario();

        var result = await UpdateProductVariantPriceRuleEndpoint.ExecuteAsync(
            scenario.Product.Id.Value,
            scenario.Variant.Id.Value,
            scenario.Rule.Id,
            new UpdateProductVariantPriceRuleRequest(" Sale ", 1, 10m, 5),
            scenario.UseCase,
            CancellationToken.None);

        Assert.IsType<NoContent>(result.Result);
        var rule = Assert.Single(scenario.Variant.PriceRules);
        Assert.Equal(scenario.Rule.Id, rule.Id);
    ...
    public async Task ExecuteAsync_InvalidRequestReturnsBadRequestWithoutSave(string invalidField)
    {
        ...
        var request = new UpdateProductVariantPriceRuleRequest("Sale", 1, 10m, 5);
        ...
        var result = await UpdateProductVariantPriceRuleEndpoint.ExecuteAsync(
            ...
            invalidField == "rule" ? Guid.Empty : scenario.Rule.Id,
    ...
    public async Task ExecuteAsync_ForwardsCancellation()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() =>
            UpdateProductVariantPriceRuleEndpoint.ExecuteAsync(
                scenario.Product.Id.Value,
                scenario.Variant.Id.Value,
                scenario.Rule.Id,
                new UpdateProductVariantPriceRuleRequest("Sale", 1, 10m, 5),
    ...
    public async Task ExecuteAsync_NullRequest_Throws() =>
        await Assert.ThrowsAsync<ArgumentNullException>(() => UpdateProductVariantPriceRuleEndpoint.ExecuteAsync(
            Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), null!, null!, CancellationToken.None));

    [Fact]
    public async Task ExecuteAsync_NullUseCase_Throws() =>
        await Assert.ThrowsAsync<ArgumentNullException>(() => UpdateProductVariantPriceRuleEndpoint.ExecuteAsync(
            Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), new("Sale", 1, 10m, 5), null!, CancellationToken.None));

    [Fact]
    public async Task ExecuteAsync_MissingRuleReturnsNotFound()
    {
        var scenario = new Scenario();
        var result = await UpdateProductVariantPriceRuleEndpoint.ExecuteAsync(
            scenario.Product.Id.Value, scenario.Variant.Id.Value, Guid.NewGuid(),
            new("Sale", 1, 10m, 5), scenario.UseCase, CancellationToken.None);
    ...
    private sealed class Scenario
    {
        public Scenario()
        {
            ...
            Rule = PriceRule.Create("Old", PriceAdjustmentType.FixedDiscount, 1m, 0);
