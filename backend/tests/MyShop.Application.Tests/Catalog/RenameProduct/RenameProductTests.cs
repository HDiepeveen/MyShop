...
namespace MyShop.Application.Tests.Catalog.RenameProduct;

public sealed class RenameProductTests
{
    [Fact]
    public async Task ExecuteAsync_RenamesProductAndSavesWithReadToken()
    {
        ...
        Assert.Equal(scenario.Command.Name, scenario.Product.Name);
    ...
    public async Task ExecuteAsync_WhenNameIsUnchanged_SucceedsWithoutSaving()
    {
        ...
        var result = await scenario.Handler.ExecuteAsync(
            scenario.Command with { Name = scenario.Product.Name },
    ...
    public async Task ExecuteAsync_WhenProductIsMissing_ReturnsFailureWithoutSaving()
    {
        ...
        Assert.Equal(RenameProductFailure.ProductNotFound, result.Failure);
        Assert.Equal(0, scenario.Products.SaveCalls);
    }
    ...
    {
        var scenario = new Scenario();
        var originalName = scenario.Product.Name;
        ...
        Assert.Equal(originalName, scenario.Product.Name);
        Assert.Equal(0, scenario.Products.SaveCalls);
    }
    ...
    private sealed class Scenario
    {
        public Scenario()
        {
            Product = Domain.Catalog.Product.Create("Original", ProductTypeId.New(), "Standard");
            Products = new ProductRepositoryFake(this);
            Handler = new UseCase(Products);
            Command = new RenameProductCommand(Product.Id, "Renamed");
        }
        ...
        public RenameProductCommand Command { get; }
    }
