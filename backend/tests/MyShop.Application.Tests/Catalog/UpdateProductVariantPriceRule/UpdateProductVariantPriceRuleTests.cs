...
namespace MyShop.Application.Tests.Catalog.UpdateProductVariantPriceRule;

public sealed class UpdateProductVariantPriceRuleTests
{
    [Fact]
    ...
    [Fact]
    public async Task ExecuteAsync_UpdatesRuleWithReadToken()
    {
    ...
    public async Task ExecuteAsync_WhenProductMissing_ReturnsFailureWithoutSave()
    {
        ...
        Assert.Equal(UpdateProductVariantPriceRuleFailure.ProductNotFound, result.Failure);
    ...
    public async Task ExecuteAsync_WhenVariantMissing_ReturnsFailureWithoutSave()
    {
        ...
        Assert.Equal(UpdateProductVariantPriceRuleFailure.VariantNotFound, result.Failure);
        Assert.Equal(0, scenario.Repository.SaveCalls);
    ...
    [Fact]
    public async Task ExecuteAsync_UnchangedNormalizedRuleDoesNotSave()
    {
        var scenario = new Scenario(true);
        var result = await scenario.UseCase.ExecuteAsync(scenario.Command with
        {
            Name = " Old ", AdjustmentType = PriceAdjustmentType.FixedDiscount,
            Value = 1m, Priority = 0, StartsAt = null, EndsAt = null
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task ExecuteAsync_MissingOrForeignRuleDoesNotMutate(bool foreignVariant)
    {
        var scenario = new Scenario(true);
        var second = scenario.Product.AddVariant("Second");
        var command = foreignVariant
            ? scenario.Command with { ProductVariantId = second.Id }
            : scenario.Command with { PriceRuleId = Guid.NewGuid() };
        var result = await scenario.UseCase.ExecuteAsync(command, CancellationToken.None);
        Assert.Equal(UpdateProductVariantPriceRuleFailure.PriceRuleNotFound, result.Failure);
        Assert.Same(scenario.Rule, Assert.Single(scenario.Variant.PriceRules));
        Assert.Equal(0, scenario.Repository.SaveCalls);
    }

    [Fact]
    public async Task ExecuteAsync_ForwardsActiveCancellationToken()
    {
        var scenario = new Scenario(true);
        using var source = new CancellationTokenSource();
    ...
    private sealed class Scenario
    {
        public Scenario(bool withSku)
        {
            ...
            Command = new(Product.Id, Variant.Id, Rule.Id, " Sale ", PriceAdjustmentType.PercentageDiscount,
        ...
        public UpdateProductVariantPriceRuleCommand Command { get; }
